    ...
    for (let i=0; i < v_stockCode.length; i++) {
        if (v_stockCode[i] !==""){
            ...
            if (result.length <= 0) {
                ...
                checkNewStock(response_stockCode.data.suggestions[0]);
            };
            let response = await axios.get(
                "https://www.twse.com.tw/exchangeReport/STOCK_DAY",
                {
                    ...
                    stockNo: v_stockCode[i],
                    },
                }
            );
            if (response.data.stat !== "OK") {throw `證交所查無此股票代碼: ${v_stockCode[i]} \n`;};
            ...
            let stockTransaction = response.data.data.map((stockData) => {
                ...
                stockData.unshift(v_stockCode[i]);
